...
beforeAll(async()=>{ directory=await mkdtemp(join(tmpdir(),'mai-pg-test-')); runtime.engine=new PGlite(directory); await runtime.engine.exec(await readFile('migrations/001-persistent-records.sql','utf8')); },30000);
