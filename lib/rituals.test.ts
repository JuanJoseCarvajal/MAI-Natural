...
describe("ritual selections",()=>{
 ...
 it("sums individual prices but blocks incomplete pricing",()=>{const chosen=baseRituals[0].productIds.map(id=>products.find(p=>p.id===id)!);expect(selectionTotal(chosen)).toBe(chosen.reduce((sum,p)=>sum+p.amountInCents,0));expect(canAddSelection(chosen,[])).toBe(false);});
