...
function OrderEditor({ order, onSaved }: { order: Order; onSaved: (order: Order) => void }) {
 ...
 return <form onSubmit={event => { event.preventDefault(); setMessage(""); setError(false); startTransition(async()=>{try { const patch = Object.fromEntries(Object.entries(draft).filter(([key,value]) => value !== original[key as keyof typeof original])); const result = await updateAdminOrder(order.id, patch); if (!result.order) throw new Error("No se pudo recuperar el pedido."); onSaved(result.order); setMessage("Cambios guardados."); } catch(e) { setError(true); setMessage(e instanceof Error ? e.message : "No pudimos guardar. Tus cambios siguen aquí."); } }); }} className="mt-5 border-t border-slate-200 pt-5">
  ...
  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{select("status", "Estado del pedido", operations)}<div className="grid gap-2 text-sm font-medium"><span>Pago · verificado por Wompi</span><p className="rounded-xl bg-slate-50 p-3">{labels[order.paymentStatus ?? "pending_confirmation"] ?? "Por verificar"}</p></div>{select("shippingStatus", "Estado del envío", operations.filter(x=>x!=="cancelled"))}<label className="grid gap-2 text-sm font-medium">Número de guía<input value={draft.trackingNumber} onChange={e=>setDraft({...draft,trackingNumber:e.target.value})} maxLength={100} disabled={pending || sandbox} className="rounded-xl border border-slate-300 p-3" placeholder="Sin guía asignada" /></label></div>
  {!sandbox && <p className="mt-3 text-sm text-slate-600">Wompi verifica el pago automáticamente. Cambiar el estado del pedido no confirma un pago. Los cambios se aplican al guardar.</p>}
...
export default function AdminOrdersManager({ initialOrders, mode = "orders" }: { initialOrders: Order[]; mode?: "orders" | "payments" | "shipping" | "sales" }) {
 ...
 return <div className="space-y-6">
  ...
  <details className="mt-4"><summary className="cursor-pointer rounded-lg py-3 font-semibold text-brand-900">Revisar y gestionar pedido</summary><ShippingQuote order={order} onSaved={updated=>setOrders(current=>current.map(item=>item.id===updated.id?updated:item))} /><WompiReview order={order} onSaved={updated=>setOrders(current=>current.map(item=>item.id===updated.id?updated:item))} /><OrderEditor order={order} onSaved={updated=>setOrders(current=>current.map(item=>item.id===updated.id?updated:item))} /></details>
  </article>)}
...
function ShippingQuote({ order, onSaved }: { order: Order; onSaved: (order: Order) => void }) {
 const [cost, setCost] = useState(String((order.shippingInCents ?? 0) / 100));
 const [message, setMessage] = useState("");
 const [pending, startTransition] = useTransition();
 if (order.paymentMethod !== "wompi") return null;
 return <form className="mt-4 rounded-xl border p-4" onSubmit={event=>{event.preventDefault(); startTransition(async()=>{try { const result=await quoteAdminOrder(order.id,Math.round(Number(cost)*100)); if(result.order)onSaved(result.order);setMessage("Cotización guardada. Comparte el enlace del pedido para que el cliente acepte el total."); }catch(error){setMessage(error instanceof Error?error.message:"No se pudo guardar.");}});}}><label className="grid gap-2">Envío confirmado (COP)<input type="number" min="0" max="1000000" step="1" required value={cost} onChange={event=>setCost(event.target.value)} disabled={pending || order.paymentStarted} className="rounded-xl border p-3" /></label><p className="my-3 text-sm">Confirma disponibilidad de los productos y costo del transporte antes de guardar. El total queda bloqueado cuando se inicia el pago.</p><button disabled={pending || order.paymentStarted} className="rounded-full border px-5 py-3 disabled:opacity-50">Guardar cotización</button><a className="ml-4 underline" href={`/checkout/result?orderId=${order.id}`} target="_blank" rel="noreferrer">Ver enlace para el cliente</a><p role="status">{message}</p></form>;
}
