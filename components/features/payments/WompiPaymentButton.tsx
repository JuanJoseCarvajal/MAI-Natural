...
export default function WompiPaymentButton({ orderId, appointmentId }: { orderId?: string; appointmentId?: string }) {
  ...
  async function pay() {
    ...
    try {
      const response = await fetch(appointmentId ? "/api/payments/wompi/appointments/checkout" : "/api/payments/wompi/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(appointmentId ? { appointmentId } : { orderId }) });
