...
export const metadata: Metadata = { title: "Resultado de prueba de asesoría", robots: { index: false, follow: false } };
export default async function Payment({ searchParams }: { searchParams: Promise<{ appointment?: string; id?: string }> }) {
  ...
  if (query.appointment && query.id) {
    try {
      ...
      if (appointment && appointment.service === initialConsultation.name && tx.reference === `mai-appointment-${appointment.id}` && tx.amount_in_cents === initialConsultation.amountInCents) {
        const labels = { APPROVED: "aprobada", DECLINED: "rechazada", PENDING: "pendiente", ERROR: "con error", VOIDED: "anulada" };
        message = `Simulación ${labels[tx.status]}. No se ha confirmado un cobro real ni la cita.`;
      }
    } catch { /* Keep the verification failure visible without exposing provider details. */ }
  }
  return <main className="mx-auto max-w-2xl px-6 py-24"><h1 className="text-3xl font-bold">Tu asesoría · resultado de Wompi</h1><p role="status" className="my-6">{message}</p><p>El equipo confirma contigo el horario, la modalidad y la recepción de una transferencia real. No envíes un segundo pago sin consultar.</p><Link className="mt-8 inline-block underline" href="/services">Volver a asesorías</Link></main>;
}
