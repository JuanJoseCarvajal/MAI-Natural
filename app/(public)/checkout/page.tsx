...
export default function CheckoutPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/orders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...fields, items: JSON.parse(signature), discountCode: discount?.valid ? discount.code : undefined }) });
  ...
  return <section className="mx-auto max-w-6xl px-5 py-12 md:py-20">
    ...
    <form onSubmit={submit} className="grid items-start gap-8 lg:grid-cols-[1.15fr_.85fr]">
      <fieldset disabled={loading} className="min-w-0 space-y-7">
        ...
        <div className="rounded-3xl border border-brand-100 bg-brand-50 p-6 md:p-8"><h2 className="text-2xl text-brand-900">03. Pago por transferencia</h2><p className="mt-3 text-sm leading-relaxed text-slate-600">Primero revisamos tu pedido y confirmamos el valor del envío. Después podrás transferir a Bancolombia y compartir tu comprobante. Tu pago se confirma cuando lo validamos.</p><p className="mt-4 text-sm font-semibold text-brand-900">No se realizará ningún cobro al enviar este formulario.</p></div>
      </fieldset>
      <aside className="rounded-3xl border border-brand-100 bg-white p-6 lg:sticky lg:top-28 md:p-8"><h2 className="text-2xl text-brand-900">Tu selección <span className="text-base text-slate-500">({items.reduce((n, item) => n + item.quantity, 0)})</span></h2>
        ...
        <button type="submit" disabled={loading || validating} className="mt-6 min-h-14 w-full rounded-full bg-brand-900 px-4 py-4 font-semibold text-white disabled:opacity-50">{loading ? "Creando tu pedido…" : "Crear pedido · sin cobro →"}</button><p className="mt-4 text-center text-xs text-slate-500">Al continuar aceptas los <Link href="/terms" className="underline">términos</Link> y la <Link href="/terms#privacidad" className="underline">política de privacidad</Link>.</p>
      </aside>
    </form>
  </section>;
}
