    ...
    <form onSubmit={submit} className="grid items-start gap-8 lg:grid-cols-[1.15fr_.85fr]">
      <fieldset disabled={loading} className="min-w-0 space-y-7">
        ...
        <div className="rounded-3xl border border-brand-100 bg-brand-50 p-6 md:p-8"><h2 className="text-2xl text-brand-900">03. ¿Cómo prefieres pagar?</h2>
          ...
          <label className={`mt-3 flex gap-3 rounded-xl border border-brand-100 bg-white p-4 ${wompiAvailable ? "cursor-pointer" : "opacity-60"}`}><input type="radio" name="payment-choice" disabled={!wompiAvailable} checked={paymentMethod.startsWith("wompi")} onChange={() => setPaymentMethod(wompiMode === "production" ? "wompi" : "wompi_sandbox")} /><span><strong>{wompiMode === "production" ? "Wompi · pago seguro" : "Wompi · entorno de pruebas"}</strong><small className="mt-1 block leading-6">{wompiAvailable && wompiMode === "production" ? "Primero confirmamos disponibilidad y envío. Después aceptas el total y pagas en Wompi." : wompiAvailable ? "Continúa al checkout seguro de Wompi para simular un pago. No realiza cobros reales." : "Wompi aún no está disponible para compras reales. Solicita tu pedido por transferencia Bancolombia."}</small></span></label>
