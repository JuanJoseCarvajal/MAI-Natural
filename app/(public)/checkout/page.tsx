...
export default function CheckoutPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (createdOrder) { await openWompiCheckout({ orderId: createdOrder.id, quoteVersion: createdOrder.quoteVersion }); return; }
    ...
    <form onSubmit={submit} className="grid items-start gap-8 lg:grid-cols-[1.15fr_.85fr]">
      <fieldset disabled={loading || Boolean(createdOrder)} className="min-w-0 space-y-7">
        ...
        <div className="rounded-3xl border border-brand-100 bg-brand-50 p-6 md:p-8"><h2 className="text-2xl text-brand-900">03. Paga con Wompi</h2>
          ...
          <p className="mt-4 text-sm font-semibold text-brand-900">Completarás el pago en la página segura de Wompi.</p></div>
      </fieldset>
