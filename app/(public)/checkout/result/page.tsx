...
export default async function CheckoutResultPage({ searchParams }: { searchParams: Promise<{ orderId?: string; id?: string }> }) {
  ...
  return <section className="mx-auto max-w-2xl px-5 py-16 md:py-24"><div className="rounded-3xl border border-brand-100 bg-white p-7 md:p-12">
    ...
    <p className="mt-5 leading-relaxed text-slate-600">{sandbox ? "Tu pedido está listo para una simulación con Wompi. No confirma un pago real ni activa la preparación del envío." : wompiOrder ? "Recibimos tu selección. Cuando confirmemos disponibilidad y envío podrás aceptar el total y pagar con Wompi desde esta página." : order ? "Recibimos tu selección. Te contactaremos para confirmar disponibilidad, envío y el valor final antes de la transferencia." : "No pudimos recuperar este pedido. Si ya lo enviaste o realizaste un pago, contacta al equipo antes de repetirlo."}</p>
    ...
    {order && wompiOrder && !query.id && !order.wompiStatus && (sandbox || order.quoteVersion) && <WompiPaymentButton mode={sandbox ? "sandbox" : "production"} quoteVersion={order.quoteVersion} orderId={order.id} />}
