...
export default async function CheckoutResultPage({ searchParams }: { searchParams: Promise<{ orderId?: string; id?: string }> }) {
  ...
  return <section className="mx-auto max-w-2xl px-5 py-16 md:py-24"><div className="rounded-3xl border border-brand-100 bg-white p-7 md:p-12">
    ...
    {order && <div className="mt-7 space-y-3 rounded-2xl bg-brand-50 p-5 text-sm"><p className="break-all"><strong><SiteText id="2a86a793d709054895fb">{"Pedido:"}</SiteText></strong> {order.id}</p><p><strong><SiteText id="ff2aea656d772c138434">{"Estado:"}</SiteText></strong> {orderStatusLabels[order.status] || "En revisión"}</p><p><strong><SiteText id="bae0a98b17ba24783d75">{"Subtotal de productos:"}</SiteText></strong> {formatCOP(order.subtotalInCents ?? order.total)}</p>{order.shippingInCents !== undefined && <><p><strong><SiteText id="e33a98401b2a32868ff7">{"Envío:"}</SiteText></strong> {formatCOP(order.shippingInCents)}</p><p><strong><SiteText id="8ab4aa9f7b4a5488370c">{"Total final:"}</SiteText></strong> {formatCOP(order.total)}</p></>}<p>{order.shippingInCents === undefined ? "Envío por confirmar. " : ""}<SiteText id="c9ea735217b1b5544451">{" Conserva tu número de pedido."}</SiteText></p></div>}
    ...
    <a href={whatsapp} target="_blank" rel="noreferrer" className="mt-8 inline-flex w-full justify-center rounded-full bg-brand-900 px-5 py-4 text-center font-semibold text-white"><SiteText id="1b87e7f9818d85906d9e">{"Consultar mi pedido por WhatsApp ↗"}</SiteText></a><Link href="/products" className="mt-5 block text-center text-sm text-brand-700 underline"><SiteText id="be02b2f4692834149316">{"Volver a la tienda"}</SiteText></Link>
  </div></section>;
}
